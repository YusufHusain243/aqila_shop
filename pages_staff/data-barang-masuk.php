...
$data_barang_masuk = show_data("SELECT 
                                ...
                                INNER JOIN pengguna ON barang_masuk.id_pengguna = pengguna.id_pengguna
                            ");
...
if (isset($_POST['tambah'])) {
    ...
    if (mysqli_query($conn, $query)) {
        echo "<script>alert('Data berhasil ditambahkan'); window.location.href = '/?page=data-barang-masuk';</script>";
    } else {
        echo "<script>alert('Error: Data gagal ditambahkan'); window.location.href = '/?page=data-barang-masuk';</script>";
    }
}
...
if (isset($_POST['edit'])) {
    ...
    if (mysqli_query($conn, $query)) {
        echo "<script>alert('Data berhasil diubah'); window.location.href = '/?page=data-barang-masuk';</script>";
    } else {
        echo "<script>alert('Error: Data gagal diubah'); window.location.href = '/?page=data-barang-masuk';</script>";
    }
}


// Hapus data
if (isset($_POST['hapus']) && isset($_POST['id_barang_masuk'])) {
    $id_barang_masuk = $_POST['id_barang_masuk'];
    $query = "DELETE FROM barang_masuk WHERE id_barang_masuk = '$id_barang_masuk'";
    if (mysqli_query($conn, $query)) {
        echo "<script>alert('Data berhasil dihapus'); window.location.href = '/?page=data-barang-masuk';</script>";
    } else {
        echo "<script>alert('Error: Data gagal dihapus'); window.location.href = '/?page=data-barang-masuk';</script>";
    }
}
?>
        ...
        <div class="col-xl-12 col-md-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#tambahEditBarangMasuk">
                        Tambah Data
                    </button>
                </div>
                <div class="card-body">
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <a class="dropdown-item text-primary" href="#" data-barang-masuk-detail='<?= json_encode($value) ?>' onclick="detailBarangMasuk(this)">
                                                            Detail
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a class="dropdown-item text-warning" href="#" data-barang-masuk-edit='<?= json_encode($value) ?>' onclick="editBarangMasuk(this)">
                                                            Edit
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a class="dropdown-item text-danger" href="#" data-barang-masuk-hapus='<?= json_encode($value) ?>' onclick="hapusBarangMasuk(this)">Hapus</a>
                                                    </li>
                                                </ul>
                                            </div>
                                        </td>
...
<!-- MODAL TAMBAH DATA -->
<div class="modal fade" id="tambahEditBarangMasuk" tabindex="-1" aria-labelledby="modalTitle" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalTitle">Tambah/Edit Barang Masuk</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <form action="/?page=data-barang-masuk" method="post" id="formBarangMasuk">
                    <input type="hidden" id="id_barang_masuk" name="id_barang_masuk" value="">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <ul class="list-group">
                    ...
                    <li class="list-group-item"><strong>Jumlah Barang Masuk :</strong> <span id="detailJumlah"></span></li>
...
<script>
    ...
    function detailBarangMasuk(el) {
        const data = JSON.parse(el.getAttribute('data-barang-masuk-detail'));
    ...
    function resetFormBarangMasuk() {
        ...
        document.getElementById('id_barang_masuk').value = '';
    ...
    // Fungsi untuk edit barang masuk
    function editBarangMasuk(el) {
        const data = JSON.parse(el.getAttribute('data-barang-masuk-edit'));

        document.getElementById('id_barang_masuk').value = data.id_barang_masuk;
        ...
        const modal = new bootstrap.Modal(document.getElementById('tambahEditBarangMasuk'));
        modal.show();
    }

    // Reset form saat modal ditutup (optional)
    const modalElement = document.getElementById('tambahEditBarangMasuk');
    modalElement.addEventListener('hidden.bs.modal', resetFormBarangMasuk);

    function hapusBarangMasuk(el) {
        const data = JSON.parse(el.getAttribute('data-barang-masuk-hapus'));
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                form.method = 'POST';
                form.action = '/?page=data-barang-masuk';

                const inputId = document.createElement('input');
                inputId.type = 'hidden';
                inputId.name = 'id_barang_masuk';
                inputId.value = data.id_barang_masuk;
